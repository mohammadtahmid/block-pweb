<?php
    ...
    class Post{
        ...
        public function AddPost($data , $file){
            ...
            if (empty($title) || empty($catId) || empty($disOne) || empty($postType) || empty($tags)) {
                $error = "Filds Must Not be Empty";
                return $error;
            }elseif($file_size > 5242835){
                $error = "File must be less than 5 mb";
                return $error;
            }elseif($file_size_two > 5242835){
                $error = "File must be less than 5 mb";
                return $error;
                ...
                if($result){
                    ...
                    $error = "Something is Wrong, Post Insert Failed!";
                    return $error;
                }
            }
        }
        ...
        public function GetAllPost($id){
            ...
            //$gp = "SELECT tbl_post.*, tbl_category.catName FROM tbl_post INNER JOIN tbl_category ON tbl_post.catId = tbl_category.catId";
        ...
        public function getPostForEdit($id){
            $get_query = "SELECT tbl_post.*, tbl_category.catName FROM tbl_post INNER JOIN tbl_category ON tbl_post.catId = tbl_category.catId WHERE tbl_post.postId = '$id'";
                ...
                if(!empty($file_name) || !empty($file_name_two)){
                    if($file_size > 5242835){
                        ...
                        if($result){
                            ...
                            $error = "Something is Wrong, Post update Failed!";
                            return $error;
                        }
                    }
                }else{
        ...
        public function DeletePost($id){
            ...
            if($del_result){
                ...
                $error = "Something is Wrong, Post Delete Failed!";
                return $error;
            }
        }
        ...
        public function deactivePost($did){
            $dq = "UPDATE tbl_post SET status = 0 WHERE postId = '$did'";
        ...
        public function activePost($aid){
            $aq = "UPDATE tbl_post SET status = 1 WHERE postId = '$aid'";
        ...
        public function sliderPost(){
            $slider_query = "SELECT tbl_post.*, tbl_category.catName, tbl_user.image, tbl_user.username FROM tbl_post INNER JOIN tbl_category ON tbl_post.catId = tbl_category.catId INNER JOIN tbl_user ON tbl_post.userId = tbl_user.userId WHERE postType = 2 AND status = 0";
        ...
        public function categoryPost($id, $offset, $limit){
            $post_query = "SELECT tbl_post.*, tbl_user.username, tbl_user.image, tbl_category.catName FROM tbl_post INNER JOIN tbl_user ON tbl_post.userId = tbl_user.userId INNER JOIN tbl_category ON tbl_post.catId = tbl_category.catId WHERE tbl_post.status = '0' AND tbl_post.catId ='$id' ORDER BY tbl_post.postId DESC LIMIT $offset, $limit";
